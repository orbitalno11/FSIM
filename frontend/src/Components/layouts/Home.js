...
const ButtonStudent = ({id, name, image, clicked}) => (
    <Fragment>
        <Grid.Column>
            ...
            <Header textalign="center" size="small">{name}</Header>
        </Grid.Column>
    </Fragment>
);

const PaneTemplate = ({label, content}) => (
    <Fragment>
        <Container>
            <Grid padded>
                <Grid.Row>
        ...
        const pane = [
            {
                ...
                render: () => (
                    <PaneTemplate label="แสดงผลการวิเคราะห์การเรียนของนักศึกษาแต่ละภาควิชาโดยสรุปออกมาเป็นกราฟ" content={
                        <Grid.Row columns={4}>
                            {pageStudent.map(item => (
                                <ButtonStudent name={item.header_modal} id={item.id_modal} image={item.Image}
                                               key={item.id_modal} clicked={() => this.openModal(item.id_modal)} />))}
                        </Grid.Row>}
                    />
                )
            },
            {
                ...
                render: () => (
                    <PaneTemplate label="แสดงผลการวิเคราะห์โครงการต่างๆในคณะวิทยาศาสตร์" content={
                        <Grid.Row>
            ...
            },
            {
                ...
                render: () => (
                    <PaneTemplate label="แสดงผลการวิเคราะห์หน้าที่การงานของนักศึกษาที่จบการศึกษา" content={
                        <Grid.Row>
                            <Grid.Column>
                    ...
                    <Grid columns={2}>
                        <Grid.Row>
                            <Grid.Column width={6}>
                                ...
                                <Button primary href="/login">จัดการข้อมูล</Button>
                            </Grid.Column>
                            <Grid.Column>
                                <Image
                                    size="massive"
